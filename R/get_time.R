...
#' @returns If x is a data.frame returns the time column, else it returns the time attribute of a SpatRaster or the shared time attribute of multiple SpatRaster.
#' @family Helper
#' @keywords internal
...
get_time <- function(x) {
  ...
  if (methods::is(x, "data.frame")) time <- x$time
