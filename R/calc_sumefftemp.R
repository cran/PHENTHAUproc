...
#' @param cf_limit - numeric - threshold under which a day is defined as cold/frost day
#' @param set - function - x is cold/frost days, a and b are parameter i.e. function (x, a, b) a + b*x
...
calc_sumefftemp <- function(x,
  ...
  # calculate sum of effective temperatures using the formula set and the cold_frost days
  # determine if day has a temperature under the threshold cf_limit
  x <- terra::ifel(x < cf_limit, 1, 0)
  x <- cumsum(x)

  # calculating sum of effective temperatures with cold_frost days and the set function
