...
#' @param from - Date - ymd - only if SpatRaster should be subset before being
#' transformed.
#' @param to - Date - ymd - only if SpatRaster should be subset before being
#' transformed.
#' @returns SpatRaster - numeric - day of year
#' @family SpatRaster transformation
#' @keywords internal
#' @description Converts logical SpatRaster with time attribute and one layer
#' per day to single layer with the day of the year (doy) where the layer is
#' 1/TRUE first.
...
convert_logical_to_doy <- function(x, from = NULL, to = NULL) {
  ...
  if (lubridate::year(from) < lubridate::year(to)) {

    from <- lubridate::ymd(paste0(lubridate::year(to), "01-01"))
